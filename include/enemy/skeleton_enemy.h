...
public:
	SkeletonEnemy()
	{
		...
		// 定义四个方向的动画帧索引
		// 每个方向6帧，从精灵图中提取
		static const std::vector<int> idx_list_up = {0, 1, 2, 3, 4, 5};		   // 向上行走的帧
		static const std::vector<int> idx_list_down = {6, 7, 8, 9, 10, 11};	   // 向下行走的帧
		static const std::vector<int> idx_list_left = {12, 13, 14, 15, 16, 17};  // 向左行走的帧
		static const std::vector<int> idx_list_right = {18, 19, 20, 21, 22, 23}; // 向右行走的帧

		// 设置正常状态下的动画
		// 每个方向设置循环播放，间隔0.1秒
		anim_up.set_loop(true);
		anim_up.set_interval(0.1);
		anim_up.set_frame_data(tex_skeleton, 6, 6, idx_list_up);
		anim_down.set_loop(true);
		anim_down.set_interval(0.1);
		anim_down.set_frame_data(tex_skeleton, 6, 6, idx_list_down);
		anim_left.set_loop(true);
		anim_left.set_interval(0.1);
		anim_left.set_frame_data(tex_skeleton, 6, 6, idx_list_left);
		anim_right.set_loop(true);
		anim_right.set_interval(0.1);
		anim_right.set_frame_data(tex_skeleton, 6, 6, idx_list_right);

		// 设置草图状态下的动画（用于预览或特殊效果）
		// 使用相同的帧索引，但使用草图纹理
		anim_up_sketch.set_loop(true);
		anim_up_sketch.set_interval(0.1);
		anim_up_sketch.set_frame_data(tex_skeleton_sketch, 6, 6, idx_list_up);
		anim_down_sketch.set_loop(true);
		anim_down_sketch.set_interval(0.1);
		anim_down_sketch.set_frame_data(tex_skeleton_sketch, 6, 6, idx_list_down);
		anim_left_sketch.set_loop(true);
		anim_left_sketch.set_interval(0.1);
		anim_left_sketch.set_frame_data(tex_skeleton_sketch, 6, 6, idx_list_left);
		anim_right_sketch.set_loop(true);
		anim_right_sketch.set_interval(0.1);
		anim_right_sketch.set_frame_data(tex_skeleton_sketch, 6, 6, idx_list_right);
